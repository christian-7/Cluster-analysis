...
filter=find(peaks(:,8)<thresh);
subsetLL=peaks(filter,1:end);

if figures==1;

figure
scatter(subsetLL(:,1),subsetLL(:,2),1)
else
end

dataDBS(:,1)=subsetLL(:,1); % x
dataDBS(:,2)=subsetLL(:,2); % y
% dataDBS(:,3)=subsetLL(:,4); % frame
...
title('Raw Data')
axis on
axis([0 max(subsetLL(:,1)) 0 max(subsetLL(:,2))])
